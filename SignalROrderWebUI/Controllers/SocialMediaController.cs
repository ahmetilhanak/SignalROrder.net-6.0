...
namespace SignalROrderWebUI.Controllers
{
    public class SocialMediaController : Controller
    {
        ...
        public async Task<IActionResult> CreateSocialMedia(CreateSocialMediaVM createSocialMediaVM)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7260/api/SocialMedia", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7260/api/SocialMedia/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7260/api/SocialMedia/GetSocialMedia/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateSocialMedia(UpdateSocialMediaVM updateSocialMediaVM)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7260/api/SocialMedia", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
