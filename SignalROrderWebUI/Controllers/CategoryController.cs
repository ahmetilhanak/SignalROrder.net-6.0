...
namespace SignalROrderWebUI.Controllers
{
    public class CategoryController : Controller
    {
        ...
        public  async Task<IActionResult> CreateCategory(CreateCategoryVM createCategoryVM)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7260/api/Category",stringContent);
             if (responseMessage.IsSuccessStatusCode)
            {
...
;           }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7260/api/Category/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
        ...
        {
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7260/api/Category/GetCategory/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateCategory(UpdateCategoryVM updateCategoryVM)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7260/api/Category", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
