...
namespace SignalROrderWebUI.Controllers
{
    public class ContactController : Controller
    {
        ...
        public async Task<IActionResult> CreateContact(CreateContactVM createContactVM)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7260/api/Contact", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7260/api/Contact/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7260/api/Contact/GetContact/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateContact(UpdateContactVM updateContactVM)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7260/api/Contact", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
