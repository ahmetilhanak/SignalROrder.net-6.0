...
namespace SignalROrderWebUI.Controllers
{
    ...
    public class AboutController : Controller
    {
        ...
        public async Task<IActionResult> CreateAbout(CreateAboutVM createAboutVM)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7260/api/About", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7260/api/About/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7260/api/About/GetAbout/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateAbout(UpdateAboutVM updateAboutVM)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7260/api/About", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
